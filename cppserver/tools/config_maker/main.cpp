...
struct enumValueInfo {
    ...
    enumValueInfo(std::string name, std::string value, std::string desc) : _name(name), _value(value), _desc(desc){}
};

struct enumInfo {
    std::string _name;
    std::vector<enumValueInfo> _evinfo;
};
...
std::map<std::string, enumInfo> enum_info_map;
std::map<std::string, structinfo> struct_info_map;

bool readEnum(const std::string & path, const std::string & struct_name, std::string & value, std::string & macro_extern, std::string & function, std::string & iconfig, std::string & oconfig) {
    ...
    while (index < sheet->rowsCount()) {
        enumInfo info;
        info._name = tools::toMultiString(sheet->cell(index++, enum_name_column).getString().c_str());
        value << "namespace " << info._name << " {\n";
        const s32 count = sheet->cell(index++, struct_count_column).getDouble();
        for (s32 j = 0; j < count; j++) {
            const std::string vname = tools::toMultiString(sheet->cell(index, enum_value_name_column).getString().c_str());
            const std::string vvalue = tools::intAsString(sheet->cell(index, enum_value_column).getDouble());
            const std::string desc = tools::toMultiString(sheet->cell(index++, enum_desc_column).getString().c_str());

            value << "    static const int " << vname << " = " << vvalue << "; /* " << desc << " */\n";
            info._evinfo.push_back(enumValueInfo(vname, vvalue, desc));
        }
        value << "};\n\n";
        index++;
        enum_info_map.insert(std::make_pair(info._name, info));
    }
    return true;
}
...
bool readExcel(const std::string & path, const std::string & struct_name, std::string & value, std::string & macro_extern, std::string & function, std::string & iconfig, std::string & oconfig) {
    ...
    try {
        ...
        for (s32 i = 0; i < sheet->columnsCount(); i++) {
            ...
            if (type != "int" && type != "string" && type != "float" && type != "bool" && type != "map" && type != "vector" && type != "set") {
                tassert(false, "file %s column %d type %s error", path.c_str(), i, type.c_str());
            }

            if (type == "map" || type == "vector" || type == "set") {
...
//         value << "}\n";
        ...
        value << "    tinyxml2::XMLDocument config;\n";
        value << "    if (tinyxml2::XML_SUCCESS != config.LoadFile(xmlpath.c_str())) {\n";
        value << "        tassert(false, \"load config %s error\", xmlpath.c_str());\n";
        value << "        return;\n";
        value << "    }\n\n";

        value << "    g_config_" << struct_name << "_map.clear();\n";

        value << "    tinyxml2::XMLElement * root = config.RootElement();\n";
        value << "    tinyxml2::XMLElement * " << struct_name << " = root->FirstChildElement(\"" << struct_name << "\");\n";
        ...
        for (s32 i = 0; i < column_names.size(); i++) {
            if (column_types[i] == "int") {
            ...
            else if (column_types[i].substr(0, 3) == "set") {
                ...
                value << "                    o" << struct_name << "._" << column_names[i] << ".insert(value);\n";
                value << "                }\n";
                value << "            }\n";
                value << "        }\n";
            }
...
bool readGlobalExcel(const std::string & path, const std::string & struct_name, std::string & value, std::string & macro_extern, std::string & function, std::string & iconfig, std::string & oconfig) {
    ...
    try {
        ...
        value << "    tinyxml2::XMLElement * root = config.RootElement();\n";
        value << "    tinyxml2::XMLElement * global = root->FirstChildElement(\"global\");\n";
        value << "    if (global) {\n";
        for (s32 i = 0; i < column_names.size(); i++) {
            if (column_types[i] == "int") {
...
int main() {
    ...
    std::string enumPath = tools::file::getApppath();
    enumPath << "/" << "enum.xls";
    if (!readEnum(enumPath, "structs", content, macro_extern, function, iconfig, oconfig)) {
    ...
    if (tools::file::getfiles(path.c_str(), ".xls", oPaths, oNames, nCount)) {
        for (s32 i = 0; i < oPaths.size(); i++) {
            ...
            } else if (oNames[i] == "structs" || oNames[i] == "enum") {
